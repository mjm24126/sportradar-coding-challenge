...
import { ingestGameData } from './handler';
...
describe('ingestGameData calls expected db methods', () => {
  let client;
  ...
  test('should call update for each row  on success', async () => {
    const result = await ingestGameData(event);
    ...
    expect(result).toEqual({"GameId": "2022020094", "event": {"GameId": "2022020094"}, "gameState": "4", "message": "ingesting ongoing 2022020094 stats"});
  });
  ...
  test('should return error on failure', async () => {
    ...
    expect(result).toEqual({
      ...
      "gameState": "4",
      "message": "error ingesting ongoing 2022020094 stats",
    });
  });
});